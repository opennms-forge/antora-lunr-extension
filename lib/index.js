...
function vendorJsFile (uiCatalog, logger, uiOutputDir, requireRequest, basename = requireRequest.split('/').pop()) {
  ...
  const jsPath = jsVendorDir + '/' + basename
  let file = uiCatalog.findByType('asset').find(({ path }) => path === jsPath)
  if (file) {
    logger.warn(`Please remove the following file from your UI since it is managed by ${packageName}: ${jsPath}`)
  } else {
    file = uiCatalog.addFile({
      type: 'asset',
      path: jsPath,
