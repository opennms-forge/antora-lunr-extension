...
function generateIndex (playbook, contentCatalog, { indexLatestOnly = false, languages = ['en'], logger } = {}) {
  ...
  const pages = contentCatalog.getPages((page) => page.out && page.asciidoc?.attributes?.noindex == null)
  ...
  const documents = pages
    ...
    .filter(({ page, $ }) => {
      ...
      if (indexLatestOnly) {
        ...
        return !(metaRobotNoIndex || notLatest)
      }
      return !metaRobotNoIndex
    })
...
function createIndexFile (index) {
  return {
    ...
    contents: Buffer.from(`lunrSiteSearch.init(lunr, ${JSON.stringify(index)})`),
