...
However, the default behavior may not be what you're used to from using other search engines.
...
If you want to search for documents that have all of the terms entered (i.e., required term presence), you must modify those terms.
To indicate that a term must be present in a matching documents, prefix the term with a `+` (e.g., `+install`).
To find a document that contains both "`install`" and "`antora`", use the following query:

    +install +antora

CAUTION: Unlike other search engines, Lunr does not support queries that are enclosed in quotes.
In the future, this extension may automatically translate a query with that syntax to a query with required term presence.
