...
<article class="doc">
  ...
  it('should only index the first document title (heading 1)', () => {
    ...
    const index = generateIndex(playbook, contentCatalog)
    const whatsNewPage = index.store[1]
...
<body class="article">
  <main role="main">
    ...
    const featuresPage = index.store[1]
...
<html lang="en">
  <body class="article">
    ...
    expect(Object.keys(index.store)).to.have.lengthOf(1)
    expect(index.store).to.have.property('1')
    expect(index.store[1].url).to.equal('/hello/1.0/features/')
  })
...
<body class="article">
  ...
  describe('Paths', () => {
    it('should use relative links when site URL is not defined', () => {
      ...
      const index = generateIndex(playbook, contentCatalog)
      expect(index.store[1].url).to.equal('/component-a/2.0/install-foo')
    })
